...
import { CURRENT_AFFAIRS_TOPICS } from './currentAffairsTopic';
...
const CurrentAffairs = () => {
  ...
  const [completedSubtopics, setCompletedSubtopics] = useState({
    nigeriahistory: Array(4).fill(false),
    governmentpolitics: Array(3).fill(false),
    currentevents: Array(4).fill(false),
  });
  ...
  const isTopicUnlocked = useCallback((index) => {
    ...
    const prevTopic = CURRENT_AFFAIRS_TOPICS[index - 1];
    ...
    currentevents: Array(4).fill(false),
      ...
      if (progressData) {
        progressData.forEach(item => {
          const topicIndex = CURRENT_AFFAIRS_TOPICS.findIndex(t => t.id === item.topic);
          if (topicIndex !== -1) {
            const subtopicIndex = CURRENT_AFFAIRS_TOPICS[topicIndex].subtopics.findIndex(
  ...
  const toggleCard = (index) => {
    if (isTopicUnlocked(index)) {
      if (expandedCard !== index && !completedSubtopics[CURRENT_AFFAIRS_TOPICS[index].id].some(Boolean)) {
  ...
  const topic = CURRENT_AFFAIRS_TOPICS.find(t => t.id === category);
  ...
  const LockedTopicModal = ({ topicIndex }) => {
    const prevTopic = CURRENT_AFFAIRS_TOPICS[topicIndex - 1];
    return (
  ...
  const UnlockPopup = ({ topicIndex, onClose }) => (
    <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white rounded-2xl p-6 max-w-md mx-auto text-center shadow-lg">
        ...
        <p className="mb-4">Well done, {firstName}! You’ve unlocked <b>{CURRENT_AFFAIRS_TOPICS[topicIndex].title}</b>. Keep up the momentum!</p>
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      <div className="container mx-auto px-4 py-4 max-w-6xl">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="space-y-8">
          {CURRENT_AFFAIRS_TOPICS.map((topic, index) => {
